...
def analyze_email(body, subject):
    """
    Send email body + subject to UIE model (PaddleNLP).
    ...
    try:
        ...
        # UIE returns a list containing a dict of extracted fields
        # Format: [{'event': [{'text': '...', 'start': X, 'end': Y, 'probability': Z}], 'date': [...]}]
        uie_result = ie(text)
        entities = uie_result[0] if uie_result else {}
        ...
        # Extract 'event'
        if "event" in entities and entities["event"]:
            result["event"] = entities["event"][0]["text"]
            
        # Extract 'time'
        if "time" in entities and entities["time"]:
            result["time"] = entities["time"][0]["text"]
            
        # Extract 'venue'
        if "venue" in entities and entities["venue"]:
            result["venue"] = entities["venue"][0]["text"]
            
        # Extract 'date'
        if "date" in entities:
            for date_match in entities["date"]:
                val = date_match["text"]
                try:
        ...
        print(f"  ERROR analyzing email with UIE: {e}")
        return {
            "type":        "unknown",
            "event":       subject,
            "dates":       [],
